...
![Seasonal daily electricity total demand in Great Britain and France](figure/fig1.png)
...
ggplot(epst, aes(x = as.POSIXct(st_halfhour, 
  ...
  ggtitle("Cooking in the UK on Saturday/Sunday/weekday (mean)
          % of episodes per halfhour") +
  geom_line() + 
...
ggplot(epst, aes(x = as.POSIXct(st_halfhour, 
  ...
  ggtitle("Eating in the UK on Saturday/Sunday/weekday (mean)
          % of episodes per halfhour") +
  geom_line() + 
  scale_colour_grey(start = 0.8, end = 0.3, guide = guide_legend(reverse=TRUE)) + 
  #scale_color_discrete(hi = "green", lo = "black") +
...
```{r weekdayEatingAtHome2005BySeason}
# only for 2005 (contemporary)
st <- svyby(~is_eating_mHome, # the variable to calculate the stat for
                   ~st_halfhour + season, # the factors
                   subset(svyMtusUKEpisodes_DTj, ba_weekday == "Weekday" & ba_survey.x == 2005), # the data in survey form
                   svymean # the stat function
                   )
...
ggplot(st, aes(x = as.POSIXct(st_halfhour, 
  ...
  ggtitle("Eating at home in the UK by season\n % of episodes per halfhour on weekdays") +
  geom_line() + 
  scale_colour_grey(start = 0.8, end = 0.3, guide = guide_legend(reverse=TRUE)) + 
  theme(legend.title = element_blank()) +
  labs(x = "Time",
...
```{r sundayCooking}
# at home
st <- svyby(~is_cooking_m, # the variable to calculate the stat for
                   ~st_halfhour + ba_survey.x, # the factors
                   subset(svyMtusUKEpisodes_DTj, ba_weekday == "Sunday"), # the data in survey form
                   svymean # the stat function
                   )
...
ggplot(st, aes(x = as.POSIXct(st_halfhour, 
  ...
  ggtitle("Cooking in the UK \n % of episodes per halfhour on Sundays") +
  geom_line() + 
  scale_colour_grey(start = 0.8, end = 0.3, guide = guide_legend(reverse=TRUE)) + 
  theme(legend.title = element_blank()) +
  labs(x = "Time",
...
XX from your DEMAND conference paper - should we include this here? XX
