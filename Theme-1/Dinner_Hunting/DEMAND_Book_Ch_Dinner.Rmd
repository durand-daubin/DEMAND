...
t <- mtusUKEpisodes_DTj[,
                    ...
                    by = .(ba_survey.x) # ba_survey pools 1983/4 and 1987
                    ][order(ba_survey.x)]

# set the names to be pretty
setnames(t, c("Survey", "N episodes", 
              "Main (% of episodes)",
              "Secondary (% of episodes)",
              "Either (% of episodes)"
              )
         )
kable(caption = "Distribution of cooking/food preparation episodes by survey year (unweighted)",
  t
  )
```
...
t[, is_cooking_m := round(100*is_cooking_m, 2)]
t[, se := NULL] # drop SE as we're going to add the 95% CI

c <- round(confint(cookEpsWm), 4) # will get * by 100 below
t <- cbind(t,c)
setnames(t, c("Sex", "Survey", "% episodes", "lci", "uci")) # must be an easier way
t[, lci := lci * 100]
t[, uci := uci * 100]
setnames(t, c("Sex", "Survey", "% episodes", "2.5%", "97.5%"))

kable(caption = "Weighted proportion of episodes reported as cooking as a main activity by men and women",
      t
      )
```
...
t[, is_eating_m := round(100*is_eating_m, 2)]
t[, se := NULL] # drop SE as we're going to add the 95% CI

c <- round(confint(eatEpsWm), 4) # will get * by 100 below
t <- cbind(t,c)
setnames(t, c("Sex", "Survey", "% episodes", "lci", "uci")) # must be an easier way
t[, lci := lci * 100]
t[, uci := uci * 100]
...
kable(caption = "Weighted proportion of episodes reported as eating as a main activity by men and women",
      t
      )
```
...
cookEpsHalfhour <- svyby(~is_cooking_m, # the variable to calculate the stat for
                   ~st_halfhour + ba_survey.x + ba_weekday, # the factors
                   svymtusUKEpisodes_DTj, # the data in survey form
                   svymean # the stat function
                   )

ggplot(cookEpsHalfhour) +
  ggtitle("Cooking: % of episodes per halfhour by day of the week (need ordering)") +
  geom_line(aes(x = as.POSIXct(st_halfhour, 
                               format = "%H:%M", 
                               tz="UTC"), 
                y = is_cooking_m, 
                fill=as.factor(ba_survey.x)
                )
            )+
  scale_fill_discrete(low="green", high="black") + 
...
```{r eatingByHalfhour}
eatEpsHalfhour <- svyby(~is_eating_m, # the variable to calculate the stat for
                   ~st_halfhour + ba_survey.x + ba_weekday, # the factors
                   svymtusUKEpisodes_DTj, # the data in survey form
                   svymean # the stat function
                   )

ggplot(eatEpsHalfhour) +
  ggtitle("Eating: % of episodes per halfhour by day of the week (need ordering)") +
  geom_line(aes(x = as.POSIXct(st_halfhour, 
                               format = "%H:%M", 
                               tz="UTC"), 
                y = is_eating_m, 
                col=as.factor(ba_survey.x)
                )
            )+
  scale_fill_discrete(low="green", high="black") + 
  theme(legend.position = "bottom") +
...
t[, weekdayLunch := round(100*weekdayLunch, 2)]
t[, se := NULL] # drop SE as we're going to add the 95% CI

c <- round(confint(weekdayLunchWm), 4) # will get * by 100 below
t <- cbind(t,c)
setnames(t, c("Survey", "% episodes", "lci", "uci")) # must be an easier way
t[, lci := lci * 100]
t[, uci := uci * 100]
setnames(t, c("Survey", "% episodes", "2.5%", "97.5%"))

kable(caption = "Weighted proportion of episodes coded as weekday lunch as a main activity",
      t
      )
```
...
t[, dinner := round(100*dinner, 2)]
t[, se := NULL] # drop SE as we're going to add the 95% CI

c <- round(confint(dinnerWm), 4) # will get * by 100 below
t <- cbind(t,c)
setnames(t, c("Survey", "% episodes", "lci", "uci")) # must be an easier way
t[, lci := lci * 100]
t[, uci := uci * 100]
...
kable(caption = "Weighted proportion of episodes coded as dinner as a main activity",
      t
      )
```
...
t[, sundayLunch := round(100*sundayLunch, 2)]
t[, se := NULL] # drop SE as we're going to add the 95% CI

c <- round(confint(sundayLunchWm), 4) # will get * by 100 below
t <- cbind(t,c)
setnames(t, c("Survey", "% episodes", "lci", "uci")) # must be an easier way
t[, lci := lci * 100]
t[, uci := uci * 100]
setnames(t, c("Survey", "% episodes", "2.5%", "97.5%"))
kable(caption = "Weighted proportion of episodes coded as sunday lunch as a main activity",       
	t       
) 
``` 
...
```{r weekdayEating}
t <- svyby(~is_eating_m, # the variable to calculate the stat for
                   ~st_halfhour + ba_survey.x + ba_eloc, # the factors
                   subset(svymtusUKEpisodes_DTj, ba_weekday == "Weekday"), # the data in survey form
                   svymean # the stat function
                   )

ggplot(t) +
  ggtitle("Eating: % of episodes per halfhour for weekdays by location") +
  geom_line(aes(x = as.POSIXct(st_halfhour, 
                               format = "%H:%M", 
                               tz="UTC"), 
                y = is_eating_m, 
