...
# main or secondary
mtusUKEpisodes_DT[, is_cooking_all:= ifelse(is_cooking_m == 100 | is_cooking_s == 100, 
                                          100, # cooking
                                          0 # not cooking
                                          )
                  ]
...
# main or secondary
mtusUKEpisodes_DT[, is_eating_all:= ifelse(is_eating_m == 100 | is_eating_s == 100, 
                                          100, # eating
                                          0 # not eating
                                          )
                  ]
...
mtusUKEpisodes_DT[, weekdayLunch := ifelse(is_eating_m == 100 & hour >= 12 & hour <= 14 & 
                                             r_dow != "Saturday" & r_dow != "Sunday",
                                                      100, # is weekday lunch
                                                      0 # is not
                                                      )
                             ]
...
# > createDinner ----
# flag dinner
mtusUKEpisodes_DT[, dinner := ifelse(is_eating_m == 100 & hour >= 17 & hour < 22,
                                                      100, # is dinner
                                                      0 # is not
                                                      )
                             ]
...
# > createSundayLunch ----
# flag sunday lunch
mtusUKEpisodes_DT[, sundayLunch := ifelse(is_eating_m == 100 & hour >= 12 & hour <= 15 &
                                            r_dow == "Sunday",
                                          100, # is sunday lunch
                                          0 # is not
                                          )
...
mtusUKHalfhourIndicator_DT[, any_cook_m := ifelse(n_main_cook_eps > 0,
                           100, # at least 1 episode of cooking
                           0)
]

mtusUKHalfhourIndicator_DT[, any_eat_m := ifelse(n_main_eat_eps > 0,
                           100, # at least 1 episode of eating
                           0)
]
...
# Join the survey to the processed episodes ----
setkey(mtusUKEpisodes_DT, ba_diarypid)
setkey(mtusUKSurvey_DT, ba_diarypid)
...
tm[, is_cooking_m := round(is_cooking_m, 2)]
tm[, se := NULL] # drop SE as we're going to add the 95% CI

cm <- round(confint(main),2)
...
ts[, is_cooking_s := round(is_cooking_s, 2)]
ts[, se := NULL] # drop SE as we're going to add the 95% CI

cs <- round(confint(sec),2)
...
# fiddle to get the right look to the table
tm <- as.data.table(main)
tm[, is_eating_m := round(is_eating_m, 2)]
tm[, se := NULL] # drop SE as we're going to add the 95% CI
...
# fiddle to get the right look to the table
ts <- as.data.table(sec)
ts[, is_eating_s := round(is_eating_s, 2)]
ts[, se := NULL] # drop SE as we're going to add the 95% CI
...
t[, is_cooking_m := round(is_cooking_m, 2)]
t[, se := NULL] # drop SE as we're going to add the 95% CI

c <- round(confint(cookEpsWm),2)
t <- cbind(t,c)
...
t[, is_eating_m := round(is_eating_m, 2)]
t[, se := NULL] # drop SE as we're going to add the 95% CI

c <- round(confint(eatEpsWm), 2) 
t <- cbind(t,c)
setnames(t, c("Sex", "Survey", "% episodes", "2.5%", "97.5%"))
...
ggplot(epst, aes(x = as.POSIXct(st_halfhour, 
  ...
  ggtitle("Cooking in the UK: % of episodes per halfhour by Saturday/Sunday/weekday (mean)") +
  geom_line() + 
...
ggplot(it, aes(x = as.POSIXct(st_halfhour, 
  ...
  ggtitle("Cooking in the UK: % of any per halfhour by Saturday/Sunday/weekday (mean)") +
  geom_line() + 
  scale_colour_grey(start = 0.8, end = 0.3, guide = guide_legend(reverse=TRUE)) + 
  #scale_color_discrete(hi = "green", lo = "black") +
  theme(legend.title = element_blank()) +
...
ggplot(epst, aes(x = as.POSIXct(st_halfhour, 
  ...
  ggtitle("Eating in the UK: % of episodes per halfhour by Saturday/Sunday/weekday (mean)") +
  geom_line() + 
  scale_colour_grey(start = 0.8, end = 0.3, guide = guide_legend(reverse=TRUE)) + 
  #scale_color_discrete(hi = "green", lo = "black") +
  theme(legend.title = element_blank()) +
...
ggplot(it, aes(x = as.POSIXct(st_halfhour, 
  ...
  ggtitle("Eating in the UK: % of any per halfhour by Saturday/Sunday/weekday (mean)") +
  geom_line() + 
  scale_colour_grey(start = 0.8, end = 0.3, guide = guide_legend(reverse=TRUE)) + 
  #scale_color_discrete(hi = "green", lo = "black") +
  theme(legend.title = element_blank()) +
...
t[, weekdayLunch := round(weekdayLunch, 2)]
t[, se := NULL] # drop SE as we're going to add the 95% CI

c <- round(confint(weekdayLunchWm), 2)
t <- cbind(t,c)
...
t[, dinner := round(dinner, 2)]
t[, se := NULL] # drop SE as we're going to add the 95% CI

c <- round(confint(dinnerWm), 2)
t <- cbind(t,c)
setnames(t, c("Survey", "% episodes", "2.5%", "97.5%"))
...
t[, sundayLunch := round(sundayLunch, 2)]
t[, se := NULL] # drop SE as we're going to add the 95% CI

c <- round(confint(sundayLunchWm), 2) 
t <- cbind(t,c)
setnames(t, c("Survey", "% episodes", "2.5%", "97.5%"))
...
```{r weekdayEating}
st <- svyby(~is_eating_m, # the variable to calculate the stat for
...
ggplot(st, aes(x = as.POSIXct(st_halfhour, 
  ...
  ggtitle("Eating in the UK: % of episodes per halfhour on weekdays by location") +
  geom_line() + 
  scale_colour_grey(start = 0.8, end = 0.3, guide = guide_legend(reverse=TRUE)) + 
  theme(legend.title = element_blank()) +
  labs(x = "Time",
