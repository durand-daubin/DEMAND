...
log using "`rpath'/BA-UK-2000-TU-analysis-location-`vers'$S_DATE.smcl", replace
...
if `do_collapse' == 1 {
	...
	* code imputed exceptions 2 and 4 to replace '-9' (see location codes above)
	* 2 = sleep (110) 
	*	[sick in bed = 120?]
	
	* 4 = working in main, second job (1110, 1210) + classes/lectures (2110)
	...
	* 0(1)32 
	...
	foreach l of numlist 0(1)32 {
			...
			* loop over days
			foreach d of numlist 1/7 {
				...
				* use non-grossing weight to correct for non response
				...
				collapse (mean) wher_*s_loc`l' [iw= `wt']
				...
				*su *_loc`l'
				save "`rpath'/data_temp/wher_`l'_s`s'_d`d'_`vers'.dta", replace
		...
		* append the others
		foreach s of numlist 1/4 {
			foreach d of numlist 1/7 { 
				qui: append using "`rpath'/data_temp/wher_`l'_s`s'_d`d'_`vers'.dta"
				erase "`rpath'/data_temp/wher_`l'_s`s'_d`d'_`vers'.dta"
			}
		}
		...
		* use zlabel to control display of legend etc
		twoway contour loc`l' ba_dayofweek time_slot_dt, by(ba_season, clegend(on pos(9))) levels(11) zlabel(#9, format(%9.1f)) name(contour_loc`l')
